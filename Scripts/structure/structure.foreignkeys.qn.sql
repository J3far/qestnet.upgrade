...
IF NOT EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.REFERENTIAL_CONSTRAINTS WHERE CONSTRAINT_NAME = 'FK_CounterMaps_Counters')
BEGIN
	ALTER TABLE [dbo].[CounterMaps] WITH CHECK ADD CONSTRAINT [FK_CounterMaps_Counters] FOREIGN KEY([CounterID]) REFERENCES [dbo].[Counters] ([QestUniqueID]) ON UPDATE NO ACTION ON DELETE NO ACTION
END
GO

IF NOT EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.REFERENTIAL_CONSTRAINTS WHERE CONSTRAINT_NAME = 'FK_CounterValues_Counters')
BEGIN
	ALTER TABLE [dbo].[CounterValues] WITH CHECK ADD CONSTRAINT [FK_CounterValues_Counters] FOREIGN KEY([QestUniqueParentID]) REFERENCES [dbo].[Counters] ([QestUniqueID]) ON UPDATE NO ACTION ON DELETE NO ACTION
END
GO
